...
```{r}
landslides <- read.csv("Global_Landslide_Catalog_Export_20250331.csv", stringsAsFactors = FALSE)
summary(landslides)
```
...
# Numeric mapping for landslide size
size_map <- c("small" = 1, "medium" = 2, "large" = 3, "very_large" = 4, "catastrophic" = 5)
landslides_subset$size_numeric <- size_map[as.character(landslides_subset$landslide_size)]
...
# Apply PCA
pca_result <- prcomp(scaled_unique)

# Create PCA DataFrame
pca_df <- as.data.frame(pca_result$x[, 1:2])
pca_df$iso_a3 <- cluster_data_unique$iso_a3
pca_df$country_name <- countrycode(pca_df$iso_a3, origin = "iso3c", destination = "country.name")
...
ggplot(tsne_df, aes(x = V1, y = V2, color = cluster, label = iso_a3)) +
  ...
  labs(
    title = "Clustering of Countries based on risk patterns",
    x = "t-SNE 1",
    y = "t-SNE 2",
    color = "Cluster"
  ) +
...
ggplot(world_clustered) +
  ...
  labs(
    title = "Landslide Risk Clusters by Country",
